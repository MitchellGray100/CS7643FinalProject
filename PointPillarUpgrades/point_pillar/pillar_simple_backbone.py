...
class SimplePillarBackbone(nn.Module):
    ...
    def __init__(self, in_channels: int, num_classes: int, base_channels: int = 32, fc1_dim: int = 256, dropout_p: float = 0.0):
        ...
        self.conv1_skip = nn.Conv2d(in_channels, base_channels, kernel_size=1, bias=False)
        self.bn1_skip   = nn.BatchNorm2d(base_channels)
        self.conv2_skip = nn.Conv2d(base_channels, base_channels * 2, kernel_size=1, bias=False)
        self.bn2_skip   = nn.BatchNorm2d(base_channels * 2)
        self.conv3_skip = nn.Conv2d(base_channels * 2, base_channels * 4, kernel_size=1, bias=False)
        self.bn3_skip   = nn.BatchNorm2d(base_channels * 4)
    ...
    def forward(self, x):
        ...
        identity = self.bn1_skip(self.conv1_skip(identity))
        out = out + identity
        ...
        identity = self.bn2_skip(self.conv2_skip(identity))
        out = out + identity
        ...
        identity = self.bn3_skip(self.conv3_skip(identity))
        out = out + identity
